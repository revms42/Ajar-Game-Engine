	...
	protected void doAction(Entity<VerAttributes> entity) {
		...
		if(xdest != xtile){
			...
			if(xoffset == 15 || xoffset == -15){
				...
				if(xdest > xtile){
					entity.getAttributes().setAttribute(VerAttribute.X_MOVE_OFFSET, xoffset+1);
					entity.getAttributes().setAttribute(
							VerAttribute.X_DRAW_POS, 
							VerAttributes.tileToCoordinate(xtile) + xoffset + 1
					);
				}else{
					entity.getAttributes().setAttribute(VerAttribute.X_MOVE_OFFSET, xoffset-1);
					entity.getAttributes().setAttribute(
			...
			if(yoffset == 15 || yoffset == -15){
				...
				if(ydest > ytile){
					entity.getAttributes().setAttribute(VerAttribute.Y_MOVE_OFFSET, yoffset+1);
					entity.getAttributes().setAttribute(
							VerAttribute.Y_DRAW_POS, 
							VerAttributes.tileToCoordinate(ytile) + yoffset + 1
					);
				}else{
					entity.getAttributes().setAttribute(VerAttribute.Y_MOVE_OFFSET, yoffset-1);
					entity.getAttributes().setAttribute(
			...
			((VerController)entity.getControllers().get(0)).done();
		}
	}
