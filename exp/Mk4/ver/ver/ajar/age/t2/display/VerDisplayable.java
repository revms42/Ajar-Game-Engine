 ...
 */
package ver.ajar.age.t2.display;
...
public class VerDisplayable extends AbstractAWTDisplayable<HashAttributes> {
	...
	public VerDisplayable(Node<HashAttributes> node) {
		super(node);
		rect = new Rectangle();
		if(node.getAttributes().getAttribute(VerAttribute.X_POS) != null){
			setPosition(node);
			setSize(node);
			initialized = true;
		}
	...
	private void setPosition(Node<HashAttributes> node){
		int x = node.getAttributes().getAttribute(VerAttribute.X_POS).intValue();
		int y = node.getAttributes().getAttribute(VerAttribute.Y_POS).intValue();
		
		rect.setLocation(x, y);
	}
	
	private void setSize(Node<HashAttributes> node){
		int w = node.getAttributes().getAttribute(VerAttribute.WIDTH).intValue();
	...
	public BufferedImage drawImage(){
		...
		Number iw = this.getAttributes().getAttribute(VerAttribute.WIDTH);
		Number ih = this.getAttributes().getAttribute(VerAttribute.HEIGHT);
		
		if(ix != null && iy != null){
			if(ix.intValue() != previousImageX || iy.intValue() != previousImageY){
				previousImage = staticImage.getSubimage(
						ix.intValue() * iw.intValue(), 
						iy.intValue() * ih.intValue(), 
						iw.intValue(), 
						ih.intValue()
				);
